...
from data import TwoStreamBatchSampler, TransformTwice
from imgaug import augmenters as iaa
from utils import AverageMeter, ProgressMeter, ToTensor, RPMFliplrToTensor, accuracy, normalize_image, parse_gpus
...
parser.add_argument('--consistency', default=100, type=float,
...
def main_worker(args):
    ...
    if not torch.cuda.is_available():
        ...
        # model_ema = model_ema.to(args.gpu[0])
        # model_ema = torch.nn.DataParallel(model_ema, args.gpu)
    ...
    # --------------------------------------------------------------------------------------------------------------
    # Create data loader
    tfs = TransformTwice(transforms.Compose([RPMFliplrToTensor(p=0.5)]))
    ...
    if args.evaluate:
        if args.evaluate_on_test:
            acc = validate(test_loader, model.module.student, class_criterion, args, valid_set="Test_s")
            acc = validate(test_loader, model.module.teacher, class_criterion, args, valid_set="Test_t")
        else:
            acc = validate(valid_loader, model.module.student, class_criterion, args, valid_set="Valid_s")
            acc = validate(valid_loader, model.module.teacher, class_criterion, args, valid_set="Valid_t")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1 = validate(valid_loader, model.module.student, class_criterion, args, valid_set="Valid_s")
        acc0 = validate(valid_loader, model.module.teacher, class_criterion, args, valid_set="Valid_t")
        ...
        if args.evaluate_on_test and is_best:
            acc2 = validate(test_loader, model.module.student, class_criterion, args, valid_set="Test_s")
            acc3 = validate(test_loader, model.module.teacher, class_criterion, args, valid_set="Test_t")
    ...
    for i, (images, target) in enumerate(data_loader):
        ...
        if args.fp16:
            with torch.cuda.amp.autocast():
                ...
                # output_nrm = model(images_nrm)
                # loss_nrm = class_criterion(output_nrm[labeled_indices], target[labeled_indices])
                # losses_nrm.update(loss_nrm.item(), labeled_indices.sum())
                
                # with torch.no_grad():
                #     output_ema = model_ema(images_ema)
                #     output_ema = output_ema.detach()
                #     loss_ema = class_criterion(output_ema[labeled_indices], target[labeled_indices])
                #     losses_ema.update(loss_ema.item(), labeled_indices.sum())
